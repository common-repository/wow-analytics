=== WOW Analytics ===
Contributors: wowanalytics
Tags: leads, tracking
Requires at least: 4.0
Tested up to: 4.4
Stable tag: 2.2
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

Embeds the WOW Analytics tracking code into your Wordpress website without having to edit your templates.

== Description ==

WOW Analytics is a powerful ‘IP Look Up’ Solution that will unmask the company names of your website visitors. WOW is an essential B2B lead generation tool that greatly enhances your online marketing ROI.

WOW Analytics will give your business detailed activity reports concerning keywords, referring sites and much more. WOW can integrate with your CRM system, automatically lead score visitors on their activities and send notifications and reports to specific sales, marketing individuals or whoever you choose based on any number of visitor variables you’ve selected .

WOW Analytics pulls further information from Linkedin.com and other business data services providing you with a detailed overview of your website visitors, including employee name and position for that specific company or organisation.

WOW Analytics will give your business an untold number of new sales opportunities and insight into who is visiting your website, allowing you to get more from your online investments, and ensure that you never lose another online prospect again.

[http://www.wowanalytics.co.uk/](http://www.wowanalytics.co.uk/)

== Installation ==

1. Upload WOW Analytics to the /wp-content/plugins/ directory
1. Activate the plugin through the 'Plugins' menu in WordPress
1. Go to the WOW Analytics panel under the 'Settings' menu and add your Client Id and set the settings you want.

== Frequently Asked Questions ==
None available

== Screenshots ==

1. Hover over the Setting menu on the left and you should see “WOW Analytics” click on this menu item 

2. WOW Analytics settings 

3. From the Admin menu at the top of WOW select “Embed Tracking” option under “Client Admin” , copy your client id

4. In Wordpress and enter the Client Id in the relevant section of the WOW Settings 


== Changelog ==

= 2.2 =
Please change to the GatorLeads plugin

= 2.1.0 =
* Added ability to users logged into Wordpress

= 2.0.6 =
* Just warn if we can't detect the wp_head method.

= 2.0.5 =
* Allow the wp_head call to be located anywhere in the <head>

= 2.0.4 =
* Resolve versioning issue

= 2.0.3 =
* Prevented plugin activation if active theme is missing the wp_head function

= 2.0.2 =
* Added code to insert a comment if the user is an admin user and output the plugin version

= 2.0.1 =
* Added code to insert a comment if the client id has not been set

= 2.0 =
* Improved the setup of the plugin, you can now use your WOW login details to set the client id

= 1.2.0 =
* Updated to use the new async tracking code

= 1.1.0 =
* Updated to use the new tracking code

= 1.0.2 =
* Fix for tracking of users.

= 1.0.1 =
* First official release

== Upgrade Notice ==

= 2.0 =
Made it easier to setup when you are unsure of your client id

= 1.0 =
First release version.